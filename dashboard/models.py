...
class RegisterSchedule(models.Model):
   name = models.CharField('Nama Gelombang/Jalur Pendaftaran', max_length=120)
   start_date = models.DateField('Tanggal Dimulai Pendaftaran')
   end_date = models.DateField('Tanggal Berakhir Pendaftaran')
   is_avail = models.BooleanField("Kuota Tersedia", default=True)

   def __str__(self):
       if self.is_avail:
           return f"{self.name} Tersedia"

       return f"{self.name} Ditutup"
...
class ParticipantRePayment(models.Model):
    ...
    payment_1 = models.FileField('Bukti Pembayaran', upload_to=user_directory_path, null=True, blank=True)
    verified_1 = models.BooleanField('Verifikasi Pembayaran', default=False)
    comment_1 = models.CharField('Komentar Pembayaran', max_length=100, null=True, blank=True)
    pay_mount_1 = models.CharField('Nominal Pembayaran', max_length=100, null=True, blank=True, help_text="Isi Dengan Angka Saja.")
    deadline_1 = models.DateField('Tengat Pembayaran', null=True, blank=True)
